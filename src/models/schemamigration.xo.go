...
// SchemaMigration represents a row from 'test_database.schema_migrations'.
...
func (sm *SchemaMigration) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO test_database.schema_migrations (` +
		`version, dirty` +
		`) VALUES (` +
		`?, ?` +
		`)`
...
func (sm *SchemaMigration) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE test_database.schema_migrations SET ` +
		`dirty = ?` +
		` WHERE version = ?`
...
func (sm *SchemaMigration) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM test_database.schema_migrations WHERE version = ?`
...
// SchemaMigrationByVersion retrieves a row from 'test_database.schema_migrations' as a SchemaMigration.
//
...
func SchemaMigrationByVersion(db XODB, version int64) (*SchemaMigration, error) {
	...
	const sqlstr = `SELECT ` +
		`version, dirty ` +
		`FROM test_database.schema_migrations ` +
		`WHERE version = ?`
